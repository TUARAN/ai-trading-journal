...
export default function CryptoPage() {
  const cryptoTrades = sampleTradingRecords.filter(trade => trade.market === 'crypto')
  const cryptoInteractions = sampleAIInteractions.filter(interaction => interaction.market === 'crypto')

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">加密市场交易记录</h1>
          <p className="text-gray-600">记录与AI交易员关于加密货币的交互和交易决策</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="card">
          ...
          <p className="text-2xl font-bold text-success-600">
            {cryptoTrades.filter(t => t.profitLoss && t.profitLoss > 0).length}
          </p>
        </div>
        <div className="card">
          <h3 className="text-sm font-medium text-gray-600">总收益</h3>
          <p className="text-2xl font-bold text-gray-900">
            ${cryptoTrades.reduce((sum, t) => sum + (t.profitLoss || 0), 0).toLocaleString()}
          </p>
        </div>
        <div className="card">
          <h3 className="text-sm font-medium text-gray-600">AI交互</h3>
          <p className="text-2xl font-bold text-gray-900">{cryptoInteractions.length}</p>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          {cryptoInteractions.map((interaction) => (
